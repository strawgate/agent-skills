...
failing_checks_json="$([[ ${#failing_checks[@]} -gt 0 ]] && json_array_from_lines "${failing_checks[@]}" || echo '[]')"
pending_checks_json="$([[ ${#pending_checks[@]} -gt 0 ]] && json_array_from_lines "${pending_checks[@]}" || echo '[]')"
focus_files_json="$([[ ${#focus_files[@]} -gt 0 ]] && json_array_from_lines "${focus_files[@]}" || echo '[]')"
blockers_json="$([[ ${#blockers[@]} -gt 0 ]] && json_array_from_lines "${blockers[@]}" || echo '[]')"
