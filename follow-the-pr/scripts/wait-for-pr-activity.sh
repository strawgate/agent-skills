...
if [[ ! -f "$SNAPSHOT_FILE" ]]; then
  ...
  startup_reasons="$(compare_snapshots "$SNAPSHOT_FILE" "$tmp_current")"
  mv "$tmp_current" "$SNAPSHOT_FILE"
  tmp_current="$(mktemp)"

  if [[ "$startup_reasons" != '[]' ]]; then
    maybe_refresh_context "$TARGET_REPO" "$TARGET_PR" "$CONTEXT_DIR"
    echo "follow-the-pr: startup activation for $TARGET_REPO#$TARGET_PR: $(jq -r 'join(",")' <<<"$startup_reasons")" >&2
    emit_summary true "$startup_reasons" "$SNAPSHOT_FILE" "$STATE_DIR" "$CONTEXT_DIR"
    exit 0
  fi
