...
Startup behavior:
- The watcher now performs an immediate startup compare against any existing `snapshot.json`.
- If the PR is already closed, changed, or otherwise actionable at startup, it exits immediately with activation reasons (it does not wait one full poll interval first).
