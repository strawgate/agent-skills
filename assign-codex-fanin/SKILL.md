...
```bash
python3 "${HOME}/.agents/skills/assign-codex-fanin/scripts/collect-cloud-artifacts.py" \
  <manifest.json>
```
...
Also remember the same reality model from fanout:

- cloud outputs were produced from the committed state on the launched branch plus the prompt text
- if a task referenced a rubric or local file that was not actually present on that branch, treat the prompt text and extracted artifacts as the real source of truth
- if several attempts report that a referenced repo-local file was missing, carry that forward explicitly in the synthesis
