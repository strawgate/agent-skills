...
ARCHIVABLE=$(echo "$ALL_SESSIONS" | jq -r '
    .[]?
...
for SESSION_NAME in $ARCHIVABLE; do
    ...
    STATE=$(echo "$ALL_SESSIONS" | jq -r --arg name "$SESSION_NAME" \
        '.[]? | select(.name == $name) | .state' 2>/dev/null)
    UPDATE_TIME=$(echo "$ALL_SESSIONS" | jq -r --arg name "$SESSION_NAME" \
        '.[]? | select(.name == $name) | .updateTime // ""' 2>/dev/null)
    ...
    if [ -n "$PR_URL" ]; then
        ...
        if [ -n "$UPDATE_TIME" ]; then
            UPDATE_EPOCH=$(jules_epoch_from_iso "$UPDATE_TIME")
    ...
    if $DRY_RUN; then
        ...
        jules_delete_session "$SESSION_ID"
        echo "    Deleted."
    fi
