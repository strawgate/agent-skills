...
2. What implementation behavior is not yet modeled/proven?
...
## Phase 3: Traceability matrix (required)
...
Status values:
- `covered-strong`: independent oracle or temporal proof + implementation checks
- `covered-partial`: some checks exist but important dimensions missing
- `uncovered`: no meaningful proof/test artifact
- `stale`: docs/spec claim coverage but code drifted

Focus first on:
- protocol state machines
- parser/wire format code
- checkpoint/ordering/ack semantics
- failure-handling and shutdown behavior
- any unsafe/perf-sensitive primitives
...
## Phase 5: Recommended fixes (implementation-ready)

For each high/critical gap, propose:
1. Exact target file/function/action.
2. Verification method split:
   - TLA+/TLC for temporal/design properties,
   - Kani for pure bounded invariants/contracts,
   - proptest for async/stateful/heap-heavy behaviors.
...
- run relevant verification commands locally before proposing PR.

## Output format

Return:
1. **Executive summary**: current coverage posture + largest risks.
...
4. **30/60/90 execution plan**:
   ...
   - 60: reducer extraction + cross-layer proof alignment,
   - 90: deeper temporal/refinement hardening.

## Quality bar

- Do not claim “covered” without naming exact artifacts.
- Mark assumptions explicitly when inferring intent from code/docs.
