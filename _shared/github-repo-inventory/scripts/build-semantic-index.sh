#!/usr/bin/env bash
# Build semantic similarity indexes for issue/PR records.
# Assumes records already exist in /tmp/issue-organizer/OWNER__REPO/
...
if [[ ! -d "$OUT_DIR/issues" ]]; then
    echo "Error: No issue records found at $OUT_DIR/issues" >&2
    echo "Run gh-triage-to-records.py first" >&2
    exit 1
fi

echo "[1/2] Building semantic indexes..."
