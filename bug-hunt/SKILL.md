...
- concurrency and timing: shutdown, retry, race, cancellation, timeout, reconnect
- data translation: parsing, serialization, normalization, schema mapping, encoding
- inconsistent behavior across two entry points that should match
- stale assumptions after refactors or cross-module moves
- "read-only" or "dry-run" paths with side effects
- "documented and supported" paths that fail in validation, preview, or preflight flows
- resource handling under repeated retry, backpressure, or partial success

## What To Skip

- speculative concerns without a reproduction
- issues that need a large redesign before you can even show the bug
- "looks ugly" findings with no user consequence
- intentionally documented behavior
- repo-specific policy disagreements disguised as bugs

## Repo-Specific Layering

This skill should stay generic.

If a repository needs specialization, create a repo-local companion skill or instructions layer that adds:
...
Do not bake those assumptions back into this generic skill unless they truly generalize.
